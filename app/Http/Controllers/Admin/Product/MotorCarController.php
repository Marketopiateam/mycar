...
class MotorCarController extends BaseController
{
    public function __construct(
        private readonly MotorCarRepositoryInterface           $motorCarRepo,
    ...
    {
        $brands = $this->motorCarRepo->getListWhere(orderBy:['id'=>'desc'],searchValue:$request->get('searchValue'), dataLimit: getWebConfig(name: 'pagination_limit'));
    ...
    {
        $brand = $this->motorCarRepo->getFirstWhere(params:['id'=>$id], relations: ['translations']);
        $language = getWebConfig(name: 'pnc_language') ?? null;
        $defaultLanguage = $language[0];
    ...
    public function updateStatus(Request $request): JsonResponse
    {
        ...
        $this->motorCarRepo->update(id:$request['id'], data:$data);
    ...
    public function delete(Request $request, BrandService $brandService): RedirectResponse
    {
        $this->productRepo->updateByParams(
            params:['brand_id'=>$request['id']],data:['brand_id' =>$request['brand_id'],'sub_category_id'=>null,'sub_sub_category_id'=>null]);
        $brand = $this->motorCarRepo->getFirstWhere(params:['id'=>$request['id']]);
        $brandService->deleteImage(data:$brand);
        $this->translationRepo->delete(model:'App\Models\Brand', id:$request['id']);
        $this->motorCarRepo->delete(params: ['id'=>$request['id']]);
    ...
    public function add(BrandAddRequest $request, BrandService $brandService): RedirectResponse
    {
        ...
        $savedAttributes = $this->motorCarRepo->add(data:$dataArray);
    ...
    public function update(BrandUpdateRequest $request, $id, BrandService $brandService): RedirectResponse
    {
        ...
        $this->motorCarRepo->update(id:$request['id'], data:$dataArray);
    ...
    {
        $brands = $this->motorCarRepo->getListWhere(searchValue:$request->get('searchValue'), dataLimit: 'all');
        $active = $this->motorCarRepo->getListWhere(filters:['status'=>1], dataLimit: 'all')->count();
        $inactive = $this->motorCarRepo->getListWhere(filters:['status'=>0], dataLimit: 'all')->count();
